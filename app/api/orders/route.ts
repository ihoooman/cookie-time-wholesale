...
export async function POST(request: Request) {
  try {
    ...
    const businessType = typeof body.businessType === "string" ? body.businessType.trim() : "کافه";
    ...
    if (businessName.length < 2) return jsonError("نام مجموعه را وارد کنید.");
    if (area.length < 2) return jsonError("منطقه یا محله مجموعه را وارد کنید.");
